...
function RealTimeEditor() {
    ...
    useEffect(() => {
        ...
        // 'codemirror'는 연결된 문서의 고유 이름임 우리가 수정해도됨
        const provider = new WebsocketProvider('ws://192.168.1.18:3333', 'codemirror', ydoc); // WebsocketProvider는 서버(여기서는 localhost:3000)와의 WebSocket 연결을 관리 -> 이 연결을 통해 문서의 변경 사항이 서버와 클라이언트 간에 실시간으로 동기화됨.
        const yText = ydoc.getText('codemirror'); //getText 메소드는 Yjs 문서 내에서 공유 텍스트 필드를 생성하거나 참조. 이 텍스트 필드는 다른 사용자와 실시간으로 텍스트를 공유하고 동기화하는 데 사용.
