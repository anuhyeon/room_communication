...
function RealTimeEditor() {
    ...
    useEffect(() => {
        ...
        const provider = new WebsocketProvider(`ws://192.168.1.18:3333/room/?roomId=${roomId}`,`codemirror_${roomId}`, ydoc); // WebsocketProvider는 서버(여기서는 localhost:3000)와의 WebSocket 연결을 관리 -> 이 연결을 통해 문서의 변경 사항이 서버와 클라이언트 간에 실시간으로 동기화됨. wss://api.poke-code.com:3333
